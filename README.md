...
### 2.3 模块划分
详细描述各个模块及其功能，例如用户管理模块、贷款申请模块、审批流程模块、风险评估模块等。

### 2.4 技术栈
列出系统使用的主要技术和工具，包括编程语言、框架、数据库、服务器等。
...
| repayType       | string  | 是   | 还款类型           | 10      | -    | 否       | "按月等额本息" |
...
| statusCd  | string  | 登录状态代码 | 40      | 否       | "成功"       |
...
| 参数名 | 类型    | 必须 | 描述     | 数据长度 | 是否非空 | 示例值          |
|--------|---------|------|----------|---------|----------|----------------|
| sysTm  | string  | 是   | 系统时间 | 30      | 否       | "2023-10-15T23:59:59" |
...
| planRepayType  | string  | 是   | 还款计划类型| 5       | 否       | "Fixed"        |
...
### 6.2 测试类型
列出需要进行的测试类型，如单元测试、集成测试、系统测试、验收测试等。

### 6.3 测试用例
详细描述测试用例，包括测试步骤、预期结果等。
